...
const Header: React.FC<HeaderProps> = ({ user, onLogout, lang, setLang, theme, setTheme, currency, setCurrency, translations }) => {
  ...
  return (
    <header className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-lg sticky top-0 z-40 shadow-sm border-b border-white/20 dark:border-slate-700/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          <div className="flex items-center">
             <AppLogo />
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="h-10 w-10 flex items-center justify-center rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-200/50 dark:hover:bg-slate-700/50 transition-colors"
            ...
            <button onClick={onLogout} className="px-4 py-2 rounded-lg text-slate-600 dark:text-slate-300 font-medium hover:bg-slate-200/50 dark:hover:bg-slate-700/50 transition-colors">
              {translations.logout}
            </button>
            <div className="hidden sm:block border-s border-slate-200/50 dark:border-slate-700 h-8"></div>
