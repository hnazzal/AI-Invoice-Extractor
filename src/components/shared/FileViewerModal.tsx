...
const FileViewerModal: React.FC<FileViewerModalProps> = ({ isOpen, onClose, fileBase64, mimeType, translations }) => {
  ...
  return (
    ...
    >
      <div 
        className="relative bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-white/30 dark:border-slate-700/50 rounded-lg shadow-xl w-full max-w-4xl h-[90vh] flex flex-col" 
      ...
      >
        ...
        <div className="p-4 flex-grow overflow-auto bg-slate-100/50 dark:bg-slate-900/50 flex items-center justify-center rounded-b-lg">
            {!objectUrl && <p className="text-slate-500">{translations.loading}...</p>}
            
            {objectUrl && mimeType.startsWith('image/') && (
                <img src={objectUrl} alt={translations.invoiceFile} className="max-w-full max-h-full mx-auto object-contain rounded-md" />
            )}
            
            {objectUrl && mimeType === 'application/pdf' && (
                <iframe src={objectUrl} title={translations.invoiceFile} className="w-full h-full border-0 rounded-md" />
            )}
