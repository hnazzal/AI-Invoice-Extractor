  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="p-4 flex-grow overflow-auto bg-slate-100 dark:bg-slate-900 flex items-center justify-center">
            {!objectUrl && <p className="text-slate-500">{translations.loading}...</p>}
            
            {objectUrl && mimeType.startsWith('image/') && (
                <img src={objectUrl} alt={translations.invoiceFile} className="max-w-full max-h-full mx-auto object-contain" />
            )}
            
            {objectUrl && mimeType === 'application/pdf' && (
                <iframe src={objectUrl} title={translations.invoiceFile} className="w-full h-full border-0" />
            )}
        ...
        <div className="px-6 py-3 border-t border-slate-200 dark:border-slate-700 flex justify-end flex-shrink-0">
            <button
                type="button"
